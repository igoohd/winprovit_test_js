import { getFormattedPost, getPosts } from "./posts";
...
test("getPosts should return a list of posts", async () => {
  ...
  expect(resultPosts).toStrictEqual(mockedPosts);
});

test("getPosts should have userId, id, title and body", async () => {
  const resultPosts = await getPosts();
...
test("should return an formatted post including propertys id, title and body", () => {
  const post = {
    userId: 1,
    id: 1,
    title:
  ...
  expect(getFormattedPost(post)).toEqual(
    expect.objectContaining({
      id: post.id,
      title: post.title,
      body: post.body,
